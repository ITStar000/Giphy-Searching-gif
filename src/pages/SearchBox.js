...
function SearchBox(){
      ...
      const [randomImageColletion, setRandomImageCollection] = useState({}); // for random api
      const [searchCollection, setSearchCollection] = useState([]);// for search api
      const [searchText, setSearchText] = useState(''); // text in search box.
      const [status, setStatus] = useState(false); // random? or search?

      // If search text is above 2 letters, status become to search method.
      //Else, it remains as random method
      ...
      //It's important. it persist status of search text and result list.
      useEffect(() =>{
      ...
      // Per 10 minutes, send get random gif.
      useEffect(()=>{
                  setInterval(() => {
                        randomApi().then(data => setRandomImageCollection(data));
                  }, 10000);
      },[]);
      // go to gif detail page.
